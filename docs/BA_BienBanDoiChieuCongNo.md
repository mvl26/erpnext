...
**Tài khoản công nợ (FR-02/FR-05 — đã chốt D7):** số liệu quét trên **TK 331 và toàn bộ tài khoản con của nó** (với NCC), **TK 131 và toàn bộ tài khoản con** (với KH) — không chỉ một tài khoản đơn lẻ. Cách lấy: tìm tài khoản gốc theo `account_number` = 331/131 của Company rồi mở rộng xuống cả cây con (nested set `lft`/`rgt`), chỉ lấy tài khoản lá; GL Entry lọc theo `account IN (cây đó)` + `party`. Không cần tra account mặc định theo từng Supplier/Customer, và **không hard-code tên tài khoản** (tên thật trên site `miyano` là `331 - Phải trả cho người bán - M`). Report đối chiếu tổng (FR-15) cũng cộng trên đúng tập tài khoản này. Hiện tại 331/131 đang là tài khoản lá, nên logic cây con là dự phòng cho khi kế toán mở chi tiết (3311, 3312…) mà không phải sửa code.
