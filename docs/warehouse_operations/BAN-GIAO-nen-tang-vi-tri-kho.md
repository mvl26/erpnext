...
Nay là **module `Warehouse Operations` trong app `erpnext`** (fork `mvl26/erpnext`), nhánh
...
Mã nguồn: `erpnext/warehouse_operations/` · Tài liệu: `docs/warehouse_operations/`
...
> đã tự thêm trước đó.
>
> Mọi file mới nằm gọn trong `erpnext/warehouse_operations/`. Chỉ **3 file của upstream bị chèn,
...
> erpnext.warehouse_operations.tests.<tên>`. **Đừng** chạy `--app erpnext` nếu chỉ muốn kiểm phần
...
>    HỘI TỤ** qua `erpnext.warehouse_operations.vitri.cay.dam_bao_cay_da_dung()` (logic dùng chung),
...
| `loai_vi_tri` có lựa chọn **"Cách ly"**, **"Trả hàng"** | bỏ cả hai (Task 7, 11/09/2026) — không nơi nào trong `warehouse_operations/vitri/` đọc hai giá trị này, nên một ô gắn "Cách ly" vẫn bị FEFO chọn ra để xuất bán như thường: **nhãn an toàn giả**. Cách ly THẬT là ranh giới tồn kho, thuộc về `Warehouse` (kho riêng), không phải một giá trị Select trên `Storage Location`. Xem `HDSD-quan-ly-vi-tri-kho.md` mục 2 cho cách làm đúng |
...
5. **Dựng `lft`/`rgt` lần đầu giờ chạy TỰ ĐỘNG và TỰ HỘI TỤ ở MỌI lần `bench migrate`**
   (`erpnext.warehouse_operations.vitri.cay.dam_bao_cay_da_dung`, gọi từ patch
