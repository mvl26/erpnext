...
Miyano ERP · module **Vi Tri Kho** trong app `erpnext` (fork `mvl26/erpnext`)
...
- Cột **Kiểm chứng** ghi **phương pháp** kiểm chứng (ISO/IEC/IEEE 29148 §9.6), không phải
  trạng thái đã kiểm:
  - `T` — kiểm thử tự động; tên module là bài test **đã có** trong `erpnext/vi_tri_kho/tests/`
    tại commit mốc.
...
Module nằm **trong app `erpnext`** (thư mục `erpnext/vi_tri_kho/`), không phải app riêng — vì
...
| VTK-KHO | `vitri/bat_kho.py`, `vitri/kho.py`, `warehouse_location_setup.*`, `public/js/vi_tri_kho/warehouse.js` | `test_bat_kho`, `test_bat_kho_xem_truoc`, `test_tat_va_dong_bo`, `test_co_quan_ly_vi_tri`, `test_warehouse_location_setup` |
...
| VTK-GAN | `doctype/item_location_preference/*`, `vitri/gan.py`, `public/js/vi_tri_kho/item.js`, `cay_chon_vi_tri.js` | `test_gan_vi_tri` |
| VTK-GY | `vitri/goi_y.py` | `test_goi_y_o` |
| VTK-LO | `doctype/batch_entry/*`, `vitri/nhap_lo.py`, `vitri/phieu_nhap.py`, `vitri/ma_vach.py`, `vitri/lo_ncc.py`, `public/js/vi_tri_kho/purchase_receipt.js`, `purchase_receipt.py` | `test_nhap_lo`, `test_loi_vao_nhap_lo`, `test_ma_vach`, `test_lo_ncc` |
| VTK-TVT | `vitri/tem.py`, `public/js/vi_tri_kho/tem_vi_tri.js`, `storage_location.js`, `storage_location_tree.js` | `test_tem` |
| VTK-TL | `vitri/nhap_lo.py` (`du_lieu_tem`, `dat_o_in_tem`), `public/js/vi_tri_kho/tem_lo.js`, `in_nhan_lo.js`, `batch.js` | `test_nhap_lo` |
| VTK-QUET | `vitri/quet.py`, `page/quet_ma_tra_cuu/*`, `public/js/vi_tri_kho/o_quet.js` | `test_quet` |
