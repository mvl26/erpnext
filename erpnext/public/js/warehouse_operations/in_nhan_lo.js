...
erpnext.warehouse_operations.in_nhan = (function () {
	// `tem_vi_tri.js` PHẢI đứng trước `tem_lo.js`: `tem_lo.may_vach()` ném lỗi
	// nếu `erpnext.warehouse_operations.tem.may_ve_ma_vach` chưa có (tem_lo.js:589).
	...
	const M_DAT_O = "erpnext.warehouse_operations.vitri.nhap_lo.dat_o_in_tem";
	const M_DU_LIEU = "erpnext.warehouse_operations.vitri.nhap_lo.du_lieu_tem";
	...
	function in_cho_cac_lo(danh_sach_lo) {
		...
		frappe.require(DUONG_NAP, () => {
			chuoi_du_lieu(danh_sach_lo).then(({ tem, khong_co_o }) => {
				...
				const mo_duoc = erpnext.warehouse_operations.tem_lo.in_xap(tem);
				if (!mo_duoc) {
