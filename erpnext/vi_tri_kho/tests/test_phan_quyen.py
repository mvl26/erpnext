...
1. `DOCTYPE_CUA_APP` bản trước là danh sách TĨNH hard-code 5 tên. Người
   ...
   filters={"module": "Vi Tri Kho"})` — thêm doctype thứ 6 vào app thì
...
def _doctype_cua_app():
	...
	return frappe.get_all("DocType", filters={"module": "Vi Tri Kho"}, pluck="name")
...
class TestKhongLoRaCong(FrappeTestCase):
	def test_khong_doctype_nao_mo_cho_customer(self):
		...
		self.assertTrue(
			doctype_cua_app,
			"truy vấn DocType theo module 'Vi Tri Kho' trả rỗng — tự nó là dấu hiệu "
...
def _report_cua_app():
	...
	ở trên."""
	return frappe.get_all("Report", filters={"module": "Vi Tri Kho"}, pluck="name")
...
class TestBaoCaoKhongMoChoCustomer(FrappeTestCase):
	...
	`frappe.get_all("Report", filters={"module": "Vi Tri Kho"})` — thêm một
	...
	def test_report_roles_khong_co_customer(self):
		...
		self.assertTrue(
			report_cua_app,
			"truy vấn Report theo module 'Vi Tri Kho' trả rỗng — tự nó là dấu hiệu truy "
